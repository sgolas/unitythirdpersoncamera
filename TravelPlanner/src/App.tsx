...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 mx-auto max-w-md relative">
      ...
      {/* Global sync button (floating, top-right) */}
      ...
      {moreOpen && (
        <div className="fixed inset-0 z-[150] bg-black/50 flex items-end" onClick={() => setMoreOpen(false)}>
          <div className="bg-white w-full max-w-md mx-auto rounded-t-3xl p-5 pb-24 animate-fadeUp" onClick={e => e.stopPropagation()}>
      ...
      {/* Bottom nav */}
      <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white border-t border-slate-200 flex z-50 shadow-lg">
        ...
        <NavTab label="Docs"    active={view === 'documents'} onClick={() => go('documents')} icon={<FileText size={22} />} />
